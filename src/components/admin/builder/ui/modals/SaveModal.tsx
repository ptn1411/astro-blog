      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold">
            {mode === 'edit'
              ? isLocal ? 'Save Changes' : 'Commit Changes'
              : isLocal ? 'Create New Page' : 'Create Page on GitHub'}
          </h3>
          ...
          {isLocal ? (
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Full path: <code className="bg-gray-100 px-1 rounded">{BASE_PATH}{fileName}</code>
              </p>
            </div>
          ) : (
            <div>
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            >
              {isSaving
                ? 'Saving...'
                : mode === 'edit'
                  ? isLocal ? 'Save Changes' : 'Commit Update'
                  : isLocal ? 'Create File' : 'Commit New Page'}
            </button>
          </div>
        </div>
      </div>
