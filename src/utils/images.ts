...
export const findImage = async (
  ...
  if (imagePath.startsWith('/src/')) {
    ...
    if (images) {
      if (typeof images[imagePath] === 'function') {
        return ((await images[imagePath]()) as { default: ImageMetadata }).default;
      }
      ...
      if (suffixMatch && typeof images[suffixMatch] === 'function') {
        return ((await images[suffixMatch]()) as { default: ImageMetadata }).default;
      }

      // Last resort: match by basename (e.g. 'me.webp')
      const basename = pathInsideAssets.split('/').pop();
      if (basename) {
        const baseMatch = keys.find((k) => k.endsWith(`/${basename}`) || k.endsWith(basename));
        if (baseMatch && typeof images[baseMatch] === 'function') {
          return ((await images[baseMatch]()) as { default: ImageMetadata }).default;
        }
      }
      ...
      try {
        console.warn('[findImage] failed to resolve', imagePath, 'candidates:', Array.from(candidates).slice(0, 20));

        console.warn('[findImage] available keys (sample):', Object.keys(images).slice(0, 30));
      } catch (_e) {
        // noop
      }
    }
  ...
  if (imagePath.startsWith('/')) {
    ...
    if (images) {
      ...
      for (const k of keys) {
        const kNorm = k.replace(/^\.\//, '/');
        if (kNorm.endsWith(imagePath) || (basename && (kNorm.endsWith(`/${basename}`) || kNorm.includes(basename)))) {
          candidates.add(k);
          candidates.add(kNorm);
        }
      }
